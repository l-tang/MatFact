"""
    Module including the implementation of matrixrix factorization for recommender system
    Date: 28/Mar/2019
    Author: Li Tang
"""
__author__ = ['Li Tang']
...
class FunkSVD:
    ...
    def train(self, penalty='ridge', penalty_weight=0.5, learning_rate=0.05, learning_rate_decay=0.85,
              min_learning_rate=None, dropout=0.0, epochs=20, early_stopping=10, matrix_p=None, matrix_q=None):
        """
                ...
                for col in range(len(self.matrix[row])):
                    ...
                    if random.random() <= 1 - dropout:
                        ...
                        if penalty == 'ridge':
                            ...
                            self.matrix_q[:, col] += learning_rate * (
                                    (self.matrix[row, col] - y_hat) * self.matrix_p[row,
                                                                      :] - penalty_weight * self.matrix_q[:, col])

                            loss += ((self.matrix[row, col] - y_hat) ** 2 + penalty_weight * (
                                    np.linalg.norm(self.matrix_p[row, :]) + np.linalg.norm(
                                self.matrix_q.T[col, :]))) / self.k
                        elif penalty == 'lasso':
                            self.matrix_p[row, :] += learning_rate * (
                                    (self.matrix[row, col] - y_hat) * self.matrix_q[:, col] - penalty_weight)
                            self.matrix_q[:, col] += learning_rate * (
                                    (self.matrix[row, col] - y_hat) * self.matrix_p[row, :] - penalty_weight)
                            loss += ((self.matrix[row, col] - y_hat) ** 2 + penalty_weight * (
